...
class DswhController extends Controller
{
    ...
    public function actionPostal()
    {
        ...
        if (in_array($event['event'], array('MessageDeliveryFailed', 'MessageDelayed', 'MessageHeld'))) {
            ...
            $messageId = isset($event['payload']['message']['message_id']) ? $event['payload']['message']['message_id'] : null;
            if (!$messageId) {
                Yii::app()->end();
            }
        ...
        if ($event['event'] == 'MessageBounced') {
            ...
            $messageId = !empty($event['payload']['original_message']['message_id']) ? $event['payload']['original_message']['message_id'] : null;
            if (!$messageId) {
                Yii::app()->end();
            }
    ...
    public function actionPostmastery()
    {
        ...
        foreach ($events as $event) {
            ...
            $deliveryLog = CampaignDeliveryLog::model()->findByAttributes(array(
                'email_message_id' => $event['header_Message-Id'],
                'status'           => CampaignDeliveryLog::STATUS_SUCCESS,
            ));
    ...
    public function actionNewsman()
    {
        ...
        foreach ($events as $event) {
            ...
            $messageId = !empty($event['data']['send_id']) ? $event['data']['send_id'] : null;
            if (empty($messageId)) {
                continue;
            }
