        ...
        <?php foreach ($server->conditions as $index => $cond) {?>
            <div class="item">
                ...
                <div class="col-lg-2 select-email-list-wrapper" style="display: <?php echo !empty($cond['list_id']) ? 'block' : 'none'; ?>">
                    <?php echo CHtml::label(Yii::t('servers', 'Email list'), 'list_id');?>
                    <?php echo CHtml::dropDownList($server->modelName . '[conditions]['.$index.'][list_id]', $cond['list_id'], $server->getCustomerEmailListsAsOptions(), $server->getHtmlOptions('conditions')); ?>
                </div>
                <div class="col-lg-2">
...
<div id="condition-template" style="display: none;">
    <div class="item">
        ...
        <div class="col-lg-2 select-email-list-wrapper" style="display: none">
            <?php echo CHtml::label(Yii::t('servers', 'Email list'), 'list_id');?>
            <?php echo CHtml::dropDownList($server->modelName . '[conditions][{index}][list_id]', '', $server->getCustomerEmailListsAsOptions(), $server->getHtmlOptions('conditions')); ?>
        </div>
        <div class="col-lg-2">
            <label><?php echo Yii::t('app', 'Action');?></label><br />
            <a href="javascript:;" class="btn btn-danger btn-flat btn-remove-condition"><?php echo IconHelper::make('delete');?></a>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <?php
                $text = 'These conditions will be applied to the email body and if matched, the given action will be taken against the email address.<br />Conditions are applied in the order they are added and execution stops at first match.';
