...
class CampaignsController extends Controller
{
    ...
    public function actionCreate($type = '')
    {
        ...
        if ($type && in_array($type, $campaign->getTypesList())) {
            $campaign->type = $type;
    ...
    public function actionUpdate($campaign_uid)
    {
        ...
        if ($request->isPostRequest) {
            ...
            if ($campaign->getIsPaused()) {
                $notify->addWarning(Yii::t('campaigns', 'Changing the campaign list/segment is not allowed when a campaign is paused!'));
                $this->redirect(array('campaigns/setup', 'campaign_uid' => $campaign->campaign_uid));
            }
    ...
    public function actionMerge_lists($campaign_uid)
    {
        ...
        $mutex    = Yii::app()->mutex;
        $lockName = 'mergeListsFor' . $campaign_uid;
    ...
    public function actionTest($campaign_uid)
    {
        ...
        foreach ($emails as $email) {
            ...
            if (!$onlyPlainText && CampaignHelper::hasRemoteContentTag($emailContent)) {
                $emailContent = CampaignHelper::fetchContentForRemoteContentTag($emailContent, $campaign);
            }
            //
            ...
            if (!empty($subscriber)) {
                ...
                // 1.5.3
                if (!$onlyPlainText && CampaignHelper::hasRemoteContentTag($_emailSubject)) {
                    $_emailSubject = CampaignHelper::fetchContentForRemoteContentTag($_emailSubject, $campaign);
                }
                //
    ...
    public function actionShare_reports_send_email($campaign_uid)
    {
        ...
        $emailBody      = $this->renderPartial('_email-share-reports', compact('shareReports'), true);
