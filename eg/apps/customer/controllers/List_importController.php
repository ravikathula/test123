...
class List_importController extends Controller
{
    ...
    protected function handleQueuePlacement($list_uid, $importClass = 'ListCsvImport')
    {
        ...
        foreach ($extensions as $ext) {
            if (is_file($finalPath . '/' . $list->list_uid . '.' . $ext)) {
                $notify->addError(Yii::t('list_import', 'The import queue already contains a file for importing into this list. Please wait for it to finish!'));
                $this->redirect(array('list_import/index', 'list_uid' => $list->list_uid));
            }
        }

        $fileName = $list->list_uid . ($importClass == 'ListTextImport' ? '.txt' : '.csv');
