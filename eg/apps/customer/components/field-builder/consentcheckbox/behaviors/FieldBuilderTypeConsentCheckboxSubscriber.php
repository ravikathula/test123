...
/**
 * FieldBuilderTypeConsentCheckboxSubscriber
 * 
...
class FieldBuilderTypeConsentCheckboxSubscriber extends CBehavior
{
    /**
     * @param CEvent $event
     * @throws CException
    ...
    protected function getValueModels()
    {
        ...
        foreach ($models as $model) {
            ...
            $valueModel->value          = !isset($appParams['POST'][$model->tag]) ? $ioFilter->xssClean($valueModel->value) : $ioFilter->xssClean($appParams['POST'][$model->tag]);
    ...
    protected function buildFieldArray($model)
    {
        ...
        $field->consent_text = str_replace('[COMPANY_NAME]', $companyName, $field->consent_text);
        
        // NOTE: maybe this should go into the view file with a display:none style ? 
        if ($field->visibility == ListFieldConsentCheckbox::VISIBILITY_VISIBLE || Yii::app()->apps->isAppName('customer')) {
