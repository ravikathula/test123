...
/**
 * FieldBuilderTypeStateSubscriber
 * 
...
class FieldBuilderTypeStateSubscriber extends CBehavior
{
    ...
    protected function buildFieldArray($model)
    {
        ...
        if ($field->required != ListField::TEXT_YES) {
            $statesList[''] = Yii::t('app', 'Please choose');
        }
        $countries = Country::model()->findAll(array('select' => 'country_id, name'));
        ...
        if ($field->visibility == ListField::VISIBILITY_VISIBLE || Yii::app()->apps->isAppName('customer')) {
            $fieldHtml = $this->owner->renderInternal($viewFile, compact('model', 'field', 'statesList'), true);
        }
