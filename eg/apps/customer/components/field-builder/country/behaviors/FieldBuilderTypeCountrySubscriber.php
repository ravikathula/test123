...
/**
 * FieldBuilderTypeCountrySubscriber
 * 
...
class FieldBuilderTypeCountrySubscriber extends CBehavior
{
    /**
     * @inheritdoc
     */
    public function attach($owner)
    {
        ...
        Yii::app()->clientScript->registerScriptFile($assetsUrl . '/field-front.js');
    ...
    protected function buildFieldArray($model)
    {
        ...
        if ($field->required != ListField::TEXT_YES) {
            $countriesList[''] = Yii::t('app', 'Please choose');
        }
        $countries = Country::model()->findAll(array('select' => 'name'));
        ...
        if ($field->visibility == ListField::VISIBILITY_VISIBLE || Yii::app()->apps->isAppName('customer')) {
            $fieldHtml = $this->owner->renderInternal($viewFile, compact('model', 'field', 'countriesList'), true);
        }
    ...
    public function _setCorrectValidationRules(CModelEvent $event)
    {
        ...
        $rules->add(array('value', 'exist', 'className' => 'Country', 'attributeName' => 'name'));
    }
