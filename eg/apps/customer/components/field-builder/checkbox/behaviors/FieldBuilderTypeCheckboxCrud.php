...
class FieldBuilderTypeCheckboxCrud extends CBehavior
{
    /**
     * @param CEvent $event
     * @throws CException
    ...
    public function _saveFields(CEvent $event)
    {
        ...
        $postModels = Yii::app()->request->getPost('ListFieldCheckbox', array());
        ...
        foreach ($postModels[$typeName] as $index => $attributes) {
            ...
            if (!empty($attributes['field_id'])) {
                $model = ListFieldCheckbox::model()->findByAttributes(array(
