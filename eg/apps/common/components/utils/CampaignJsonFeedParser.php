...
class CampaignJsonFeedParser
{
    ...
    public static function parseContent($content, Campaign $campaign, ListSubscriber $subscriber = null, $cache = false, $cacheKeySuffix = null, DeliveryServer $server = null)
    {
        ...
        foreach ($multiMatches[0] as $fullFeedHtml) {
            ...
            $feedItems = self::getRemoteFeedItems($attributes['url'], $count, $campaign, $doCache, $offset, $daysBack, $noItemAction);
            ...
            // since 1.5.1
            $html = !empty($html) ? $html : $feedItemTemplate;
            foreach ($feedItems as $index => $feedItem) {
                foreach ($feedItemsMap as $tag => $mapValue) {
                    if (!isset($feedItem[$mapValue]) || !is_string($feedItem[$mapValue])) {
                        continue;
     ...
     * @throws Exception
     */
    public static function getRemoteFeedItems($url, $count = 10, Campaign $campaign, $cache = false, $offset = 0, $daysBack = -1, $noItemAction = '')
    {
        $cacheKey = sha1(sprintf('m:%s.c:%s.u:%s.c:%s.o:%s.d:%s.s:%s', __METHOD__, $campaign->campaign_uid, $url, $count, $offset, $daysBack, $campaign->send_at));
        ...
        if ($cache) {
            ...
            Yii::app()->cache->set($cacheKey, $items, $cacheTTL);
        }
