...
class ListSubscriber extends ActiveRecord
{
    ...
    public function sendCreatedNotifications()
    {
        ...
        if (MW_IS_CLI) {
            $context = Yii::app()->getCommandRunner()->getCommand();
        } else {
        ...
        $emailBody     = $context->renderFile(Yii::getPathOfAlias('frontend.views.lists._email-subscriber-created') . '.php', $viewData, true);
    ...
    public function getFullData()
    {
        ...
        if (!empty($this->optinHistory)) {
            foreach ($optinData as $key => $value) {
                ...
                $_data[$tag] = $this->optinHistory->$key;
            }
        }
