...
class Campaign extends ActiveRecord
{
    ...
    public function rules()
    {
        $rules = array(
            ...
            array('campaign_uid, customer_id, group_id, list_id, name, type, status, search_recurring, search_ar_event, search_ar_time', 'safe', 'on'=>'search'),
        );
            ...
            'sort'  => array(
                'attributes' => array(
                    ...
                    'search_ar_time' => array(
                        'asc'  => 'CONCAT(option.autoresponder_time_value, " ", option.autoresponder_time_unit) ASC',
                        'desc' => 'CONCAT(option.autoresponder_time_value, " ", option.autoresponder_time_unit) DESC',
                    ),
                ),
                'defaultOrder'  => array(
    ...
    public function saveStatus($status = null)
    {
        ...
        try {
            ...
            $result = Yii::app()->getDb()->createCommand()->update($this->tableName(), $attributes, 'campaign_id = :cid', array(':cid' => (int)$this->campaign_id));
    ...
    public function sendStatsEmail()
    {
        ...
        $emailBody      = $renderer->renderFile(Yii::getPathOfAlias('console.views.campaign-stats') . '.php', $viewData, true);
        $emailTemplate  = str_replace('[CONTENT]', $emailBody, $emailTemplate);
        ...
        $emailParams['subject'] = Yii::t('campaign_reports', 'The campaign {name} has finished sending, here are the stats', array('{name}' => $this->name));
