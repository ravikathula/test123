...
class SuppressionListImportCommand extends ConsoleCommand
{
    ...
    protected function processCsv(array $params)
    {
        ...
        try {
            $deleteSql = 'DELETE FROM {{customer_suppression_list_email}} 
              WHERE email_id IN(
                SELECT email_id FROM (
                  SELECT email_id, COUNT(*) cnt FROM {{customer_suppression_list_email}} 
                    WHERE list_id = :lid GROUP BY email_md5 HAVING cnt > 1
                ) t1
            )';
            Yii::app()->db->createCommand($deleteSql)->execute(array(
                ':lid' => $list->list_id,
            ));
        } catch(Exception $e) {
            Yii::log($e->getMessage(), CLogger::LEVEL_ERROR);
